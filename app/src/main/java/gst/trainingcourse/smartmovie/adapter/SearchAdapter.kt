...
import gst.trainingcourse.smartmovie.model.ListGenres
import gst.trainingcourse.smartmovie.model.Result
import kotlinx.android.synthetic.main.item_search.view.*

class SearchAdapter : RecyclerView.Adapter<SearchAdapter.CategoryViewHolder>() {
    ...
    inner class CategoryViewHolder(onClickItem: (Result) -> Unit, itemView: View) :
        ...
        fun bindItemCategory(result: Result) {
            Glide.with(itemView).load("https://image.tmdb.org/t/p/w342" + result.poster_path)
                .into(itemView.ivSearch)
            itemView.tvTitleSearch.text = result.title

            var genres = ""
            val originalList = ListGenres.getListGenres()
            val listIdGenres = result.genre_ids
            val n = listIdGenres.size
            for (i in 0 until n) {
                if (i == n - 1) {
                    if (listIdGenres[i] in originalList) {
                        genres += originalList[listIdGenres[i]]
                    }
                } else {
                    if (listIdGenres[i] in originalList) {
                        genres += "${originalList[listIdGenres[i]]} | "
            ...
            itemView.rbSearch.rating = (result.vote_average / 2).toFloat()
        }
    }
}